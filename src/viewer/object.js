...
var _object = React.createClass({
	...
	render:function(){
		...
		return (
			<div className={znui.react.classname("zr-json-viewer-base zr-json-viewer-object", (this.state.fold?'fold':'unfold'))} style={znui.react.style(this.props.style)}>
				{
				...
				<div className="field-warp object-warp">
					<div className="meta-data">
						<span className="fold-icon" onClick={()=>this.setState({ fold: !this.state.fold })}><SVGIcon icon={(this.state.fold?'faCaretRight':'faCaretDown')} /></span>
						{this.__renderLabel()}
						{
